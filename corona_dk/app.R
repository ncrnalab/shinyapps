      ...
      column (12,
              wellPanel (                
                HTML (paste ("<font size=5>National and regional data on corona spread from <a href='https://www.ssi.dk/sygdomme-beredskab-og-forskning/sygdomsovervaagning/c/covid19-overvaagning/arkiv-med-overvaagningsdata-for-covid19'>SSI</a>.</font><br/>",
                             "<font size=4>Data is updated weekdays at 2pm (CET).", 
                             "P-values are calculated by simple binomial tests with number of positive (successes),", 
...
ggtheme <- theme_bw() +
  theme (
    ...
    legend.title = element_text(size = 16, face = NULL, color = "black"),
...
server <- shinyServer(function(input, output, session) {
  ...
  ld = list()
  df.regional = data.frame()
  df.national = data.frame()
  
  rv <- reactiveValues(
    df.regional.recent = data.frame() # the most recent regional data (where is corona now?)
  )
  
  
  labeller <- c(pct_pos = "Positive fraction of tests", ntested = "Number of tests")
  
  plegend <- c("TRUE" = "Corona detected (p<0.05)", "FALSE" = "Maybe just false positives", "NA" = "N/A")
  
  pcolor <- structure (c("#d62728", "#2ca02c", "grey"), names = as.character (plegend))
  
  
  get_recent_regional <- function (specificity = 0.995) {
    
    
    if (nrow (df.regional) == 0) {
      return (data.frame())
    } 
    ...
    df.recent <- df.regional %>% filter (date == recent_date) %>%
      ...
      mutate (p=b.test (npositive, ntested, 1-specificity))
    
    df.recent
    
  }  
  
  get_recent_regional_reactive <- reactive({
    
    get_recent_regional(input$specificity)
  ...
  withProgress(message = 'Reading data', value = 0, {
    ...
    rdata <- paste ("data/", ld[[1]]$date, ".Rdata", sep="")
    
    if (file.exists (rdata)) {
      
      cat(file=stderr(), "data loaded from previous session...", "\n")
      load (rdata)
      
      
    } else {
      
      cat(file=stderr(), "process national data...", "\n")
      
      # process tests data only for most recent dataset
      
      ld[1] <- map (ld[1], function (data) {
        
        print (data$file)    
        df.tests <- read.table(unz(data$file, "Test_pos_over_time.csv"), sep=";", header=T, dec=",", colClasses="character")
        
        colnames (df.tests) <- c("date", "npositive", "not_prev_pos", "pos_pct", "prev_pos", "ntested", "cumulative_ntested")
        ...
        data$df.tests <- df.tests
        
        data
      })
      
      df.national <- ld[[1]]$df.tests
      
      # make in future process?
      ...
      cat(file=stderr(), "saving data...", "\n")
      
      save (ld, df.national, df.regional, file=rdata)
      
    }
    
    cat(file=stderr(), "extracting latest regional test results...", "\n")
    
    region_vct <- unique (ld[[1]]$df.regional$municipal_name)
  ...
  output$national <- renderPlotly({
    ...
    g <- ggplot () + 
      ...
      geom_point(data=data.m %>% filter (var == "pct_pos"), aes (x=date, y=val, color = legend)) + 
  ...
  output$notification <- renderText ({
    ...
    df.recent.regional <- get_recent_regional_reactive () %>% filter (p < 0.05)
    ...
    if (nrow (df.recent.regional) < 10) {
      
      alert <- paste (image, "<font size=4><b>Alert</b>: Corona recently detected in", paste (df.recent.regional$municipal_name, collapse=", "), "</font>")
      
    } else {
      
      alert <- paste (image, "<font size=4><b>Alert</b>: Corona recently detected in", paste (df.recent.regional$municipal_name[1:10], collapse=", "), "to name a few...</font>")
  ...
  output$regional_text <- renderText ({
    ...
    df.region <- df.regional %>% filter (tolower(municipal_name) == tolower(input$region)) %>% filter (!is.na (p))
...
# min (which ((1- cumsum (dbinom (1:2848, size=2848, prob=0.005))) < 0.05))
